...
func WriteSchemaFile(conv *internal.Conv, now time.Time, name string, out *os.File, driver string) {
	...
	spDDL := ddl.GetDDL(ddl.Config{Comments: true, ProtectIds: false, NamedSchemas: true, Tables: true, ForeignKeys: true, SpDialect: conv.SpDialect, Source: driver}, conv.SpNamedSchemas, conv.SpSchema, conv.SpSequences)
	if len(spDDL) == 0 {
	...
	spDDL = ddl.GetDDL(ddl.Config{Comments: false, ProtectIds: true, NamedSchemas: true, Tables: true, ForeignKeys: true, SpDialect: conv.SpDialect, Source: driver}, conv.SpNamedSchemas, conv.SpSchema, conv.SpSequences)
	if len(spDDL) == 0 {
		spDDL = []string{"\n-- Schema is empty -- no tables found\n"}
	}
	l = []string{
