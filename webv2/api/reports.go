...
func GetDSpannerDDL(w http.ResponseWriter, r *http.Request) {
	...
	now := time.Now()
	spDDL := ddl.GetDDL(ddl.Config{Comments: true, ProtectIds: false, Tables: true, ForeignKeys: true, SpDialect: conv.SpDialect, Source: sessionState.Driver}, conv.SpNamedSchemas, conv.SpSchema, conv.SpSequences)
	if len(spDDL) == 0 {
